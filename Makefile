...
build: build-cleanup
	cd src && npm i --only=prod && zip -r ../dist/http_worker-event-driven.zip ./
...
update-lambda-fn: build
	aws lambda update-function-code --function-name http_worker-event-driven --zip-file fileb://$(shell pwd)/dist/http_worker.zip --publish | jq .FunctionArn
